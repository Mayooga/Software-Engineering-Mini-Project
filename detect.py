...
def detect_violations_live(video_path: str, output_path: str,
                ...
                if viol_types:
                    ...
                    for v in viol_types:
                        if v not in tracker.violations[tid]:
                            ...
                            # Track basic lane statistics for suggestions
                            if v in ("speeding", "risky", "phone"):
                                if cx < W / 3:
                                    lane = "left"
                                elif cx < 2 * W / 3:
                                    lane = "middle"
    ...
    # Derive simple, human-readable suggestions for risk reduction
    suggestions = []
    total_events = len(tracker.violation_log)
    duration_sec = (TOTAL / FPS) if FPS and TOTAL else 0
    viol_per_min = (total_events / (duration_sec / 60.0)) if duration_sec >= 30 else total_events

    if total_viols["speeding"] > 0:
        suggestions.append(
            "Frequent speeding detected; consider stronger speed enforcement or speed calming measures on this road segment."
        )
        # Lane‑specific speeding focus
        left_s  = lane_counts["left"]["speeding"]
        mid_s   = lane_counts["middle"]["speeding"]
        right_s = lane_counts["right"]["speeding"]
        max_s   = max(left_s, mid_s, right_s)
        if max_s >= 2:
            if max_s == left_s:
                suggestions.append("More speeding observed in the left lanes; add clearer speed signage or targeted checks there.")
            elif max_s == right_s:
                suggestions.append("More speeding observed in the right lanes; focus enforcement vehicles or cameras on that side.")

    if total_viols["risky"] > 0:
        suggestions.append(
            "Multiple risky driving maneuvers (weaving / unstable lanes) detected; review lane markings, merging areas, and physical dividers."
        )

    if total_viols["phone"] > 0:
        suggestions.append(
            "Instances of possible phone use while driving detected; plan awareness campaigns and distracted‑driving checks."
        )

    if viol_per_min >= 5:
        suggestions.append(
            "Overall violation density is high for the video duration; consider continuous monitoring or patrol presence at this location."
        )
    elif total_events == 0:
        suggestions.append(
            "Few or no violations observed in this sample; existing control measures may be effective for this time period."
        )
